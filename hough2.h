 ...
 * NOTE: if distance is over 180B:, take the "other" distance (short path)
 */
...
void print_classifier(void) {
	...
	for (i = 0; i < _classifier.length; i++) {
        ...
        //display_printf(0,0,"worko");
		printf("(%.2fB:, %.2f) from %i lines\n", t/_precision, r, l);
...
void plot_point(sizep_t index) {
	...
	// Going through 180B: is enough to cover all directions
